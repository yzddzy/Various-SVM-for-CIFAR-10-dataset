...
########################################################################
    # scikit-learn实现polynomial kernel svm
    poly = SVC(kernel='poly', degree=3, C=1.5, gamma=1/330)
    poly.fit(H_train, Y_train)

    # 输出分类正确率
    Y_pred = poly.predict(H_test)
    accuracy = accuracy_score(Y_test, Y_pred)
    print("test_accuracy: {:.2f}%".format(accuracy * 100))

    # 输出support vectors总数以及正负样本数
    n_sv = poly.n_support_
    print("support vectors:", np.sum(n_sv))
    sign = poly.dual_coef_[0]
    ...
    # 分别可视化正负样本α最大的20图
    selected_pos_sv = np.argsort(poly.dual_coef_[0])[::-1][:20]

    fig, axs = plt.subplots(nrows=4, ncols=5, figsize=(32, 32))
    axs = axs.ravel()
    for i, idx in enumerate(selected_pos_sv):
        alpha = poly.dual_coef_[0][idx]
        img = X_train[poly.support_[idx]]
        axs[i].imshow(img)
        axs[i].axis('off')
        axs[i].set_title('α={:.2f}'.format(alpha))
    plt.show()

    selected_neg_sv = np.argsort(-poly.dual_coef_[0])[::-1][:20]
